...
CREATE PROCEDURE Update_Employment_Status /*will probably need to use an upcoming procedure to help us but if someone has an idea do write*/
@Employee_ID int
AS
GO
...
DELETE FROM Attendance 
where EXISTS(
SELECT official_day_off FROM Employee
...
DELETE FROM Attendance
WHERE EXISTS(
SELECT final_approval_status from leave l
