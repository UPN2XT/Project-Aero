...
RETURNS TABLE AS RETURN (
    SELECT performance_ID, rating, comments, semester
FROM Performance
...
BEGIN
    ...
    IF EXISTS (SELECT type_of_contract
    FROM Employee
    ...
    VALUES
        (@ReqID, @employee_ID, @replacement_emp);
...
BEGIN
    ...
    BEGIN
        ...
        VALUES
            (@ReqID, @employee_ID, @document_description, @file_name, 'valid', 'Unpaid', GETDATE());
    END
...
BEGIN
    ...
    FROM Document
    WHERE document_ID = @request_ID AND status = 'valid') THEN 'approved' 
        ELSE 'rejected' 
    END
    WHERE Emp1_ID = @Upperboard_ID AND Leave_ID = @request_ID;
END
