...
BEGIN
    ...
    ELSE
        BEGIN
        INSERT INTO Holiday
            (holiday_name, from_date, to_date)
        VALUES
            (@holiday_name, @from_date, @to_date);
    END
END;
...
BEGIN
    ...
    -- Basic validation
    IF @Emp1_ID IS NULL OR @Emp2_ID IS NULL BEGIN
        PRINT 'Emp1_ID and Emp2_ID must not be NULL.';
        RETURN;
    END
    IF @from_date IS NULL OR @to_date IS NULL BEGIN
        PRINT 'from_date and to_date must not be NULL.';
        RETURN;
    END
    IF @from_date > @to_date BEGIN
        PRINT 'from_date must be less than or equal to to_date.';
        RETURN;
    END
    IF @Emp1_ID = @Emp2_ID BEGIN
        PRINT 'Emp1_ID and Emp2_ID cannot be the same.';
        RETURN;
    END
    -- Verify employees exist
    IF EXISTS (SELECT 1
    FROM Employee
    WHERE employee_id = @Emp1_ID) SET @Emp1Exists = 1;
    IF EXISTS (SELECT 1
    FROM Employee
    WHERE employee_id = @Emp2_ID) SET @Emp2Exists = 1;
    IF @Emp1Exists = 0 BEGIN
        PRINT 'Employee with Emp1_ID not found.';
        RETURN;
    END
    IF @Emp2Exists = 0 BEGIN
        PRINT 'Employee with Emp2_ID not found.';
        RETURN;
    END
    ...
    INSERT INTO dbo.Employee_Replace_Employee
...
where request_ID = @request_ID)
    ...
    if @annual_balance >= @num_days
        ...
        insert into Employee_Replace_Employee
            (Emp1_ID,Emp2_ID,from_date, to_date)
        values
            (@emp_ID, @replacement_id, @start_date_acc, @end_date_acc)
...
select @emplid=emp_ID, @date_original_day=date_of_original_workday, @replacementID=replacement_emp
from Compensation_Leave
where @request_ID=request_ID
...
select @compensationDate=start_date
from leave
where request_ID=@request_ID
...
select @total_duration=total_duration
from Attendance
where date=@date_original_day and emp_ID=@emplid
...
insert into Employee_Replace_Employee
values
    (@emplid, @replacementID, @compensationDate, @compensationDate)
...
select @days_num = count(attendance_ID)
from Attendance
...
where un.Emp_ID = @employee_ID
    ...
    select @request_id = l.request_ID
    from Unpaid_Leave un
        inner join Leave l
        on l.request_ID = un.request_ID
    where un.Emp_ID = @employee_ID
...
BEGIN
    ...
    --getting Overtime Factor based on role , Role (can have more than 1 role)
    select top 1
        @percentage_overtime=r.percentage_overtime
    from Employee_Role er join role r on r.role_name=er.role_name
    where emp_ID=@employee_ID
    order by r.rank
    --calculating Extra Hours + Required Hours = (no of attended sessions * hours per day )
    select @countRecords=count(attendance_ID),
    ...
    --added seconds
    from Attendance
    where  emp_ID=@employee_ID and status='attended' and month(date)=month(current_timestamp) and
...
CREATE OR ALTER FUNCTION get_employee_managed_by_hr (@hr_id INT)
RETURNS @ResultTable TABLE (
    -- Define the structure of the returned table here
    employee_ID INT,
    name VARCHAR(255)
)
AS
BEGIN
    ...
    IF EXISTS(SELECT 1
    FROM Employee_Role
    WHERE emp_ID = @hr_id AND role_name = 'HR Manager')
    BEGIN
        ...
        FROM Employee
        WHERE dept_name = 'HR' AND employee_ID <> @hr_id;

        RETURN;
    -- Exit early
    END

    -- Logic 2: HR Representative
    DECLARE @role_name VARCHAR(50);

    SELECT TOP 1
        @role_name = r.role_name
    FROM Employee_Role r
    WHERE r.emp_ID = @hr_id AND r.role_name LIKE 'HR_Representative_%';
...
values
    (getdate(), @final_Salary, @from, @to, @comments, @bonus, @total_deduction, @employee_ID)
--Finalize Deductions
...
Begin
    ...
    IF EXISTS (
        SELECT 1
    FROM Employee E
    WHERE employee_id=@employee_ID AND password=@password and dept_name <> 'HR')
        SET @success =1
    ELSE
        SET @success =0;
    Return @success
...
values
    (CAST(CURRENT_TIMESTAMP AS DATE), @start_date, @end_date)
...
values
    (@req_id, @employee_ID, @replacement_emp)
...
if @role_name like'HR_Representative_%'
        begin
    select @mang_id=emp_ID
    from Employee_Role
    where role_name='HR Manager'
...
select @HR_Representative_id=emp_ID
from Employee_Role
where role_name='HR_Representative_'+ @dept_name
...
if @onleave =1
    ...
    select @repl_id= emp2_id
    from Employee_Replace_Employee
    where Emp1_ID=@HR_Representative_id and CAST(CURRENT_TIMESTAMP AS DATE) between from_date and to_date
    insert into Employee_Approve_Leave
        (Emp1_ID, leave_ID )
    values
        (@repl_id, @req_id)--@repl_id
end
    else
        insert into Employee_Approve_Leave
    (Emp1_ID, leave_ID )
values
    (@HR_Representative_id, @req_id)--@HR_Representative_id
...
-- search for emp_id of vice dean in the same department as the employee applying for this leave
        ...
        insert into Employee_Approve_Leave
            (Emp1_ID, leave_ID )
        values
            (@Vicedean_id, @req_id)-- @Vicedean_id --1
    end
            else
                insert into Employee_Approve_Leave
        (Emp1_ID, leave_ID )
...
BEGIN
    ...
    from Employee_Role
    where role_name like '%HR%Manager%'
...
declare @current_status varchar(50)
SELECT @current_status = final_approval_status
FROM Leave
WHERE request_ID = @request_ID;
...
SELECT @contract=type_of_contract
from Employee
...
INSERT INTO Document
    (type, description, file_name, creation_date,emp_ID,medical_ID)
...
select @approval_emp_ID1 = emp_ID
from Employee_Role
...
begin
    ...
    if(@emp_role = 'Lecturer' or @emp_role = 'Teaching Assistant')
        ...
        if(@onleave = 1)
                begin
            select @vice_dean_id = employee_id
            from Employee e
                inner join Employee_Role er
                on e.employee_id = er.emp_ID
...
INSERT INTO Document
    (type, description, file_name, creation_date, emp_ID,unpaid_ID)
...
if (EXISTS(
    ...
    not exists(select *
    from Unpaid_Leave U inner join Employee E on U.Emp_ID=E.employee_id
...
INSERT INTO Compensation_Leave
    (request_ID, reason, date_of_original_workday, emp_ID, replacement_emp)
...
INSERT INTO Performance
    (rating, comments,emp_ID, semester)
VALUES(@rating, @comment, @employee_ID, @semester)
GO

CREATE OR ALTER PROC init_values
AS
